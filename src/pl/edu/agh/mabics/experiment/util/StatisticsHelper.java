...
public class StatisticsHelper {
    ...
    public void prepareStatisticFiles(Set<SimulationResult> simulationResults, int numberOfGames, String outputDirName) {
        initDirectory(outputDirName);
        prepareStatisticFile(simulationResults, numberOfGames, TIME_OF_LAST_GETTER_NAME, outputDirName + "\\" + TIME_OF_LAST_STATISTIC_FILE_NAME);
        prepareStatisticFile(simulationResults, numberOfGames, COLLISIONS_GETTER_NAME, outputDirName + "\\" + COLLISIONS_NUMBERS_STATISTIC_FILE_NAME);
        prepareStatisticFile(simulationResults, numberOfGames, AVERAGE_TIME_GETTER_NAME, outputDirName + "\\" + AVERAGE_TIMES_STATISTIC_FILE_NAME);

    }


    public void prepareStatisticFile(Set<SimulationResult> simulationResults, int numberOfGames, String valueGetterName, String fileName) {
        ...
        for (int i = 0; i < numberOfGames; i++) {
            ...
            try {
                writeLine(fileWriter, i + 1, new String[]{mean.toString(), new Double(mean - std).toString(), new Double(mean + std).toString()});
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public String[] getStatisticFilesNames() {
        return new String[]{AVERAGE_TIMES_STATISTIC_FILE_NAME, TIME_OF_LAST_STATISTIC_FILE_NAME, COLLISIONS_NUMBERS_STATISTIC_FILE_NAME};
    }
}
